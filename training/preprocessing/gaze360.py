"""
Processes the Gaze360 dataset by cropping face and eye regions from each image,
converting 3D gaze vectors to 2D yaw/pitch angles,
and saving the results in a structured directory format with corresponding label files.

Main Steps:
- Loads metadata and image paths from the .mat file provided by Gaze360.
- Iterates through all samples, cropping face and eye images using bounding boxes.
- Converts 3D gaze direction to 2D yaw/pitch angles.
- Saves cropped images and writes label files for each data split.

Outputs:
- Cropped face images (224x224), left/right eye images (60x36), and label files for each split.
- Label files contain image paths and gaze information for each sample.

Note:
- The script does not use geometric normalization; all cropping is based on bounding boxes.
"""
...
root = "data/raw/Gaze360" # input dataset path (contains metadata.mat and imgs/)
out_root = "data/preprocessed/Gaze360" # output path
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess the Gaze360 dataset for training."
    )
    parser.add_argument(
        "--raw-dir",
        default=root,
        help="Path to the raw Gaze360 dataset (contains metadata.mat and imgs/).",
    )
    parser.add_argument(
        "--output-dir",
        default=out_root,
