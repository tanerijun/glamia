"""
Processes the MPIIFaceGaze dataset by normalizing face and eye images, transforming gaze and head pose data, and saving the results in a structured directory format with corresponding label files.

Main Steps:
- Iterates through all subjects and their sample lists.
- Loads camera calibration and annotation data for each subject.
- For each sample:
    - Loads the image and decodes annotation fields.
    - Normalizes the image, gaze, and head pose using geometric normalization (via the core.norm class).
    - Crops and equalizes left/right eye images.
    - Flips images and gaze/head pose if processing the right eye.
    - Converts gaze and head pose to 2D angles.
    - Saves processed images and writes all relevant information to a label file.

Outputs:
- Normalized face images (224x224), left/right eye images (60x36), and label files per subject.
- Label files contain image paths, gaze/head pose info, and normalization parameters for each sample.
"""
...
# Paths are relative to the training/ directory. Override via CLI arguments.
# Note: the sample list ships with MPIIGaze (not MPIIFaceGaze); download both datasets.
root = "data/raw/MPIIFaceGaze"
sample_root = "data/raw/MPIIGaze/evaluation_subset/sample_list_for_eye_image"
out_root = "data/preprocessed/MPIIFaceGaze"
scale = True
...
def ImageProcessing_Person(im_root, anno_path, sample_list, im_outpath, label_outpath, person):
    ...
    for count, info in enumerate(im_list):
        ...
        _ = int(im_name.split(".")[0]) # im_number
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess the MPIIFaceGaze dataset for training."
    )
    parser.add_argument(
        "--raw-dir",
        default=root,
        help="Path to the raw MPIIFaceGaze dataset.",
    )
    parser.add_argument(
        "--sample-list-dir",
        default=sample_root,
        help="Path to the MPIIGaze evaluation-subset sample list directory.",
    )
    parser.add_argument(
        "--output-dir",
        default=out_root,
        help="Directory to write the preprocessed dataset.",
    )
    args = parser.parse_args()

    root = args.raw_dir
    sample_root = args.sample_list_dir
