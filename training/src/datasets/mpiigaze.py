...
class MPIIGazeDataset(Dataset):
    def __init__(self, data_root, split, num_bins, angle_range, image_size=224):
        ...
        self.split = (
            split  # For MPII, split can be 'train', 'val', or a person ID like 'p00'
        )
            ...
            self.transform = transforms.Compose(
                [
                    transforms.RandomResizedCrop(
                        size=image_size, scale=(0.8, 1.2), ratio=(0.9, 1.1)
                    ),
        ...
        # The MPIIFaceGaze dataset is filtered during preprocessing to be within ~42 degrees.
        self.lines = lines_raw
        print(
            f"Loading {len(self.lines)} samples for split '{self.split}' from {label_file}..."
        )
    ...
    def __getitem__(self, idx):
        ...
        pitch_binned = np.floor(
            (pitch_deg + self.angle_range / 2) / self.bin_width
        ).astype(int)
        yaw_binned = np.floor((yaw_deg + self.angle_range / 2) / self.bin_width).astype(
            int
        )
