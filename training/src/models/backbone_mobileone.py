...
class MobileOneBlock(nn.Module):
    def __init__(
        ...
        if inference_mode:
            self.reparam_conv: nn.Conv2d | None = nn.Conv2d(
                in_channels=in_channels,
            ...
            self.rbr_skip: nn.BatchNorm2d | None = (
            ...
            self.rbr_scale: nn.Sequential | None = None
            if kernel_size > 1:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        conv_out: int | torch.Tensor = 0
        for branch in self.rbr_conv:
            conv_out = conv_out + branch(x)

        return self.activation(
            self.se(cast(torch.Tensor, conv_out) + scale_out + identity_out)
        )
...
class MobileOneBackbone(nn.Module):
    PARAMS: dict[str, dict[str, Any]] = {
        "mobileone_s0": {
            "num_blocks": [2, 8, 10, 1],
            "width": [0.75, 1.0, 1.0, 2.0],
            "conv_branches": 4,
            "use_se": False,
        },
        "mobileone_s1": {
            "num_blocks": [2, 8, 10, 1],
            "width": [1.5, 1.5, 2.0, 2.5],
            "conv_branches": 1,
            "use_se": False,
        },
        "mobileone_s2": {
            "num_blocks": [2, 8, 10, 1],
            "width": [1.5, 2.0, 2.5, 4.0],
            "conv_branches": 1,
            "use_se": False,
        },
        "mobileone_s3": {
            "num_blocks": [2, 8, 10, 1],
            "width": [2.0, 2.5, 3.0, 4.0],
            "conv_branches": 1,
            "use_se": False,
        },
        "mobileone_s4": {
            "num_blocks": [2, 8, 10, 1],
            "width": [3.0, 3.5, 3.5, 4.0],
            "conv_branches": 1,
            "use_se": True,
        },
    }
    ...
    def __init__(
        self, arch: str, pretrained: bool = False, inference_mode: bool = False
    ):
        super().__init__()
        if arch not in self.PARAMS:
            raise ValueError(
                f"Unsupported MobileOne architecture: {arch}. Available: {list(self.PARAMS.keys())}"
            )
    ...
    def _load_pretrained(self, arch: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to load pretrained weights for {arch} from {url}. Error: {e}"
            )
