...
class Mapper:
    ...
    def __init__(
        ...
        spatial_gate_threshold: float | None = None,
    ):
        """
        ...
        Args:
            ...
            spatial_gate_threshold: Spatial outlier gate.
        """
        ...
        if spatial_gate_threshold is not None and spatial_gate_threshold <= 0:
            raise ValueError("spatial_gate_threshold must be positive or None.")
        self.spatial_gate_threshold = spatial_gate_threshold
    ...
    def add_dynamic_calibration_point(
        ...
        Returns:
            bool: True if the point was accepted into the buffer, False if it
            was rejected by the spatial gate.
        """
        ...
        if self.spatial_gate_threshold is not None and self.is_trained:
            pred_x, pred_y = self.predict(feature_vector)
            offset = float(np.hypot(pred_x - target_point[0], pred_y - target_point[1]))
            if offset > self.spatial_gate_threshold:
                return False
